...
export default function ArchivedTickets() {
  ...
  // 1. Filtrer les signalements pour n'avoir que les "Fermé"
  // On utilise useMemo pour éviter de recalculer à chaque rendu si 'signalements' ne change pas
  const seulementFerme = useMemo(() => {
    return signalements.filter((sign: any) => sign.statut?.nameStatut === "Fermé");
  }, [signalements]);

  // 2. Préparer les données du graphique seulement les Ferme 
  const chartData = useMemo(() => {
    const counts: Record<string, number> = {};
    seulementFerme.forEach((sign) => {
      ...
      <main className="p-8 flex flex-row gap-8">
        ...
        <div className="w-2/3">
          ...
          <ul className="space-y-4">
            ...
            {seulementFerme.length === 0 && (
              <p className="text-gray-500 italic">Aucun signalement fermé pour le moment.</p>
            )}
          </ul>
        </div>
